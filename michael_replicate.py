...
def main():
	...
	old_files_location='/global/scratch/mmartin2/CAROLINE/finite/steel_reflected/HM50'

	#Define the location for the perturbed temperature files
	new_files_location='/global/scratch/lpgregg/burnup_safety/michael_replicate'

	#copy over all the input files.
	new_input_filename="hm50_finite_input"

	for i in range(0,len(case_folder_names)):
		mkdir_p(new_files_location);
		mkdir_p(new_files_location+"/"+case_folder_names[i])
		case_folder_names[i]
		old_file_path=old_files_location+"/"+case_folder_names[i]+"/"+case_file_names[i]
		new_file_path=new_files_location+"/"+case_folder_names[i]+"/"+new_input_filename
		print(old_file_path)
		print(new_file_path)
